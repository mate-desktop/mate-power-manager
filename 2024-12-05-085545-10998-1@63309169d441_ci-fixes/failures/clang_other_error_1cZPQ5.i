# 1 "gpm-backlight-helper.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 397 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "gpm-backlight-helper.c" 2
# 23 "gpm-backlight-helper.c"
# 1 "../config.h" 1
# 24 "gpm-backlight-helper.c" 2
...
# 32 "gpm-backlight-helper.c" 2
# 43 "gpm-backlight-helper.c"
static gchar *
gcm_backlight_helper_get_best_backlight (void)
{
 ...
 for (i=0; backlight_interfaces[i] != ((void*)0); i++) {
  filename = g_build_filename ("/sys/class/backlight",
          backlight_interfaces[i], ((void*)0));
 ...
 dir = g_dir_open ("/sys/class/backlight", 0, &error);
 if (dir == ((void*)0)) {
  g_log ("PowerManager", G_LOG_LEVEL_WARNING, "failed to find any devices: %s", error->message);
  g_error_free (error);
  goto out;
 }
...
main (gint argc, gchar *argv[])
{
 ...
 gboolean get_brightness = (0);
 gboolean get_max_brightness = (0);
 gchar *filename = ((void*)0);
 gchar *filename_file = ((void*)0);
 gchar *contents = ((void*)0);

 const GOptionEntry options[] = {
  { "set-brightness", '\0', 0, G_OPTION_ARG_INT, &set_brightness,

    gettext ("Set the current brightness"), ((void*)0) },
  { "get-brightness", '\0', 0, G_OPTION_ARG_NONE, &get_brightness,

    gettext ("Get the current brightness"), ((void*)0) },
  { "get-max-brightness", '\0', 0, G_OPTION_ARG_NONE, &get_max_brightness,

    gettext ("Get the number of brightness levels supported"), ((void*)0) },
  { ((void*)0), 0, 0, G_OPTION_ARG_NONE, ((void*)0), ((void*)0), ((void*)0) }
 };


 setlocale (6, "");
 bindtextdomain ("mate-power-manager", "/usr/local/share/locale");
 bind_textdomain_codeset ("mate-power-manager", "UTF-8");
 textdomain ("mate-power-manager");

 context = g_option_context_new (((void*)0));

 g_option_context_set_summary (context, gettext ("MATE Power Manager Backlight Helper"));
 g_option_context_add_main_entries (context, options, ((void*)0));
 g_option_context_parse (context, &argc, &argv, ((void*)0));
 g_option_context_free (context);


 if (set_brightness == -1 && !get_brightness && !get_max_brightness) {

  g_print ("%s\n", gettext ("No valid option was specified"));
  retval = 3;
  goto out;
 }
 ...
 if (filename == ((void*)0)) {
  ...
  g_print ("%s\n", gettext ("No backlights were found on your system"));
  retval = 4;
  goto out;
 }
 ...
 if (get_brightness) {
  filename_file = g_build_filename (filename, "brightness", ((void*)0));
  ret = g_file_get_contents (filename_file, &contents, ((void*)0), &error);
  if (!ret) {

   g_print ("%s: %s\n", gettext ("Could not get the value of the backlight"), error->message);
   g_error_free (error);
   retval = 3;
   goto out;
  }
 ...
 if (get_max_brightness) {
  filename_file = g_build_filename (filename, "max_brightness", ((void*)0));
  ret = g_file_get_contents (filename_file, &contents, ((void*)0), &error);
  if (!ret) {

   g_print ("%s: %s\n", gettext ("Could not get the maximum value of the backlight"), error->message);
   g_error_free (error);
   retval = 3;
   goto out;
  }
 ...
 if (pkexec_uid_str == ((void*)0)) {
  ...
  g_print ("%s\n", gettext ("This program must only be run through pkexec"));
  retval = 4;
  goto out;
 }
 ...
 if (set_brightness != -1) {
  ...
  if (!ret) {
   ...
   g_print ("%s: %s\n", gettext ("Could not set the value of the backlight"), error->message);
   g_error_free (error);
   retval = 3;
   goto out;
  }
